...
function TaskAccordion({ task, onEdit }) {
      ...
      {open && (
        <div className="px-5 pb-4 border-t border-zinc-800 pt-3 space-y-3">
          ...
          <div className="flex flex-wrap gap-4 text-xs text-zinc-500">
            {task.assigned_to && (
              <span>👤 Assigned to: <span className="text-zinc-300">{task.assigned_to}</span></span>
            )}
            {task.due_date && (
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-xs text-zinc-400 hover:text-white border border-white/[0.08] hover:border-white/20 px-3 py-1.5 rounded-lg transition active:scale-[0.98] cursor-pointer"
          >
      ...
      <div className="w-full max-w-2xl space-y-10">
        ...
        {tasks.length > 0 && (
          <div className="space-y-3">
            ...
            {tasks.map((t) => (
              <TaskAccordion key={t.id} task={t} onEdit={setEditingTask} />
            ))}
          </div>
        )}
      </div>
